...
| `wait` | `"idle"` \| `"text"` \| `"prompt"` \| `"until"` | idle: `min_idle` (number 0–60, default 0.2); text: `pattern` (required, **Python** regexp), `buffer` (string); until: `pred` (required, an elisp predicate polled until non-nil; an elisp error fails the wait — wrap in `ignore-errors` if an error means "not yet"), `buffer`; all: `timeout` (10). Options on the wrong wait kind are rejected. |
